...
### Fixed
- SSH connections no longer break after many command executions: exec channels are now closed after each command, preventing server-side `MaxSessions` exhaustion (`ChannelException: Administratively prohibited`) (#14)
- Stale connections recover automatically by reconnecting and retrying once when opening a session fails (#14)

### Changed
- Enabled SSH transport keepalive (30s) so idle connections survive firewall/sshd idle timeouts (#14)
- README: removed uv installation instructions, uncollapsed the Claude Code and Codex sections (#15)
